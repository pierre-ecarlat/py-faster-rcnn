...
#
# Modified for personal dataset (From PASCAL VOC)
# Pierre Ecarlat
# --------------------------------------------------------
...
import subprocess
import uuid
from foodinc_reduced_eval import foodinc_reduced_eval
from fast_rcnn.config import cfg

class foodinc_reduced(imdb):
    def __init__(self, image_set, year, devkit_path=None):
        imdb.__init__(self, 'foodinc_reduced_' + year + '_' + image_set)
        self._year = year
    ...
    def _get_default_path(self):
        ...
        """
        return os.path.join(cfg.DATA_DIR, 'Foodinc_reduced_' + self._year)
    ...
    def gt_roidb(self):
        ...
        gt_roidb = [self._load_foodinc_reduced_annotation(index)
    ...
    def _load_foodinc_reduced_annotation(self, index):
        """
    ...
    def _get_foodinc_reduced_results_file_template(self):
    ...
    def _write_foodinc_reduced_results_file(self, all_boxes):
        for cls_ind, cls in enumerate(self.classes):
            if cls == '__background__':
                continue
            print 'Writing {} Foodinc reduced results file, ID: {}'.format(cls, cls_ind)
            filename = self._get_foodinc_reduced_results_file_template().format(cls_ind)
...
#                continue
            if cls == '__background__':
                continue
            filename = self._get_foodinc_reduced_results_file_template().format(i)
            rec, prec, ap = foodinc_reduced_eval(
