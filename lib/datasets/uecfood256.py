...
#
# Modified for EUC Food-256 Dataset (From PASCAL VOC)
# Thomas McLachlan
# --------------------------------------------------------
...
import subprocess
import uuid
from uecfood_eval import uecfood_eval
...
#                       'min_size'    : 2}
        ...
        assert os.path.exists(self._devkit_path), \
                'UEC Food 256 devkit path does not exist: {}'.format(self._devkit_path)
    ...
    def _get_uecfood_results_file_template(self):
        # VOCdevkit/results/VOC2007/Main/<comp_id>_det_test_aeroplane.txt
        filename = self._get_comp_id() + '_det_' + self._image_set + '_{:07d}.txt'
        path = os.path.join(
            self._devkit_path,
            'results',
            'UECFOOD256_' + self._year,
            filename)
        return path

    def _write_uecfood_results_file(self, all_boxes):
        for cls_ind, cls in enumerate(self.classes):
            if cls == '__background__':
                continue
            print 'Writing {} UECFOOD256 results file, ID: {}'.format(cls, cls_ind)
            filename = self._get_uecfood_results_file_template().format(cls_ind)
...
#                continue
            if cls == '__background__':
                continue
            filename = self._get_uecfood_results_file_template().format(i)
            rec, prec, ap = uecfood_eval(
    ...
    def evaluate_detections(self, all_boxes, output_dir):
        self._write_uecfood_results_file(all_boxes)
